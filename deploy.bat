...
echo [Step 1] Reading App Versions...
for /f "delims=" %%V in ('python -c "import re; content=open(r'%BASE_DIR%\setup.py', encoding='utf-8').read(); m=re.search(r'version=\x22([^\x22]+)\x22', content); print(m.group(1) if m else '')"') do set SETUP_VER=%%V
for /f "delims=" %%V in ('python -c "import re; content=open(r'%BASE_DIR%\jmstudio\app_config.py', encoding='utf-8').read(); m=re.search(r'VERSION\s*=\s*\x22([^\x22]+)\x22', content); print(m.group(1) if m else '')"') do set APP_VER=%%V
