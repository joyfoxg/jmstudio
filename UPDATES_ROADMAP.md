...
## 🎉 현재까지 완료 및 추가된 주요 기능 (v3.9.0 ~ v3.9.29)
...
*   **[✔] Obsidian 100% 호환 인피니트 캔버스(Infinite Canvas) 및 SVG 연결선 에지(Edge) 플로팅 툴바 탑재 (v3.9.26)**
*   **[✔] 폴더 임베딩 카드(디렉토리 뷰어) 및 네이티브 다른 이름으로 저장(Save As) 대화상자 연동 완료 (v3.9.26)**
*   **[✔] 양방향 위키 링크(Bi-directional Wiki Links) 및 듀얼 백링크 네비게이터 패널 구현 완료 (v3.9.26)**
*   **[✔] 지식 그래프(Force Graph) 내 카테고리별 문서 성격 아이콘(이모지) 매핑 및 2D Canvas 시각화 완료 (v3.9.26)**
*   **[✔] 되돌리기(Undo) / 다시실행(Redo) CodeMirror 6 공식 커맨드 매핑 및 런타임 오류 완벽 해결 (v3.9.26)**
*   **[✔] 내 서재 미디어 파일 별도 분리(내 미디어 아코디언), 확장자별 전용 아이콘 매핑, 미리보기 연동 및 저장 차단 안전 장치 구현 (v3.9.27)**
*   **[✔] 캔버스 다중 선택(Multi-select), 영역 선택, 그룹화, 빠른 복제 및 마크다운 편집 전용 단축키 전면 구현 (v3.9.28)**
*   **[✔] esm.sh CDN 컴파일 실패(HTTP 500) 방지용 버전 고정 및 중복 인스턴스 충돌 제거, favicon 404 해결 (v3.9.29)**
...
## 🗺️ 차기 추천 업데이트 우선순위 (Recommended Upcoming Updates)

제품 경쟁력 강화와 사용자 편의 극대화를 위한 신규 개발 우선순위입니다.

### 1순위: 🌌 Obsidian 호환 인피니트 캔버스 (Infinite Canvas) - [✔] 완료 (v3.9.26)
*   **세부 내용**: 
    *   [✔] 옵시디언 오픈소스 표준 스펙을 준수하는 `.canvas` JSON 파일 파서 구현.
    *   [✔] **D3-Zoom** 기반 무한 패닝/줌 및 SVG 관계선(Bezier Curve) 렌더링.
    *   [✔] 카드 노드 내부에 CodeMirror 6 에디터, 이미지/PDF 뷰어 융합 렌더링.
*   **기대 효과**: 선형 문서에서 2차원 지식 맵으로의 전환으로 협업 및 브레인스토밍 기능 비약적 향상.

### 2순위: 🖼️ 드래그 앤 드롭 이미지 자동 로컬 호스팅 및 붙여넣기 (`Ctrl+V`)
*   **세부 내용**: 
    *   에디터 내 복제/드롭된 이미지를 `assets/images/` 폴더에 타임스탬프 기반 고유 파일명으로 자동 저장.
    *   해당 마크다운 이미지 링크(`![](...)`) 자동 인서트 기능.
*   **기대 효과**: 외부 호스팅 의존성을 줄이고 자료 수집 및 캡처 삽입 동선 극대화.

### 3순위: 🎓 Zotero & BibTeX 학술 참고문헌 인용 자동화
*   **세부 내용**: 
    *   서재 내 `references.bib` 참고문헌 데이터 자동 파싱 및 `[@citation_key]` 에디터 자동완성 팝업 연동.
    *   인쇄 및 PDF 컴파일 시 하단에 IEEE/APA 규격 참고문헌 목록 자동 포맷 생성.
*   **기대 효과**: 학술 및 논문 작성 전용 도구로서의 고유성 공고화.

### 4순위: 📊 Plotly.js 동적 차트 & 3D 결정구조/오비탈 시각화
*   **세부 내용**: 
    *   마크다운 데이터 테이블의 Plotly.js 인터랙티브 차트(Scatter, 3D Plot 등) 시각화.
    *   재료/화학 논문용 `.cif` 및 `.xyz` 분자 파일 3D 대화형 뷰어 모듈 탑재.
*   **기대 효과**: 캡처본을 대체하는 차세대 입체 데이터 시각화 제공.

### 5순위: 🤖 AI Research Assistant & 수식 OCR 비전 스캔
*   **세부 내용**: 
    *   로컬 LLM (Ollama) 및 OpenAI API 연동을 통한 수식/SMILES 도출 및 논문 분석 비서 패널 신설.
    *   수식 이미지 복사-붙여넣기 시 LaTeX 코드로 자동 변환하는 Vision OCR 기능.

---

## 📈 장기 로드맵 검토 과제 (Backlog Items)
1. **[✔] Typora식 위지윅(WYSIWYG) 하이브리드 실시간 편집 모드**: 프리뷰와 에디터를 단일 통합 뷰로 제공 (v3.9.21 완료).
