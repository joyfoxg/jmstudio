...
def get_asset_path(relative_path):
    """ Get absolute path to resource, works for dev, pip installation and PyInstaller """
    # package_dir은 jmstudio 패키지 설치 경로
    package_dir = os.path.dirname(os.path.abspath(__file__))
    # PyInstaller 환경일 경우
    if hasattr(sys, '_MEIPASS'):
        # sys._MEIPASS 바로 아래에 에셋이 추가된 경우 (예: --add-data "frontend;frontend")
        direct_path = os.path.join(sys._MEIPASS, relative_path)
