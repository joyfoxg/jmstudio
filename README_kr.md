...
* **캔버스 단축키**: 모든 주요 단축키(다중 선택, 그룹화, 복제, 마크다운 서식 등)는 하단의 [⌨️ Keyboard Shortcuts (단축키 안내)](#-keyboard-shortcuts-단축키-안내) 섹션에서 자세히 확인할 수 있습니다.

### 14. 📂 내 미디어(My Media) 아코디언 분리, 전용 아이콘 및 이진 파일 보호 락 (v3.9.27)
* **내 미디어(My Media) 사이드바 독립**: "내 서재" 목록에서 그림, PDF, 오디오, 비디오 등의 바이너리 미디어 파일들을 제외하고, 하단의 전용 "내 미디어" 아코디언 메뉴로 분리 렌더링하여 관리 편의성을 극대화합니다. 폴더 구조는 양쪽 영역 모두 완벽히 유지됩니다.
* **미디어 확장자별 전용 아이콘**: 그림(`image`), 비디오(`video`), 음악(`music`) 등 미디어 파일의 확장자를 자동으로 판독해 파일 탐색기 내부에 알맞은 고유 아이콘을 부여합니다.
* **읽기 전용 미디어 미리보기**: 미디어 파일을 열었을 때, 원본 에디터 입력을 비활성화(투명도 0.5 및 클릭 차단)하고 에디터를 읽기 전용으로 전환하는 동시에, 우측 미리보기 패널에 해당 미디어의 전용 플레이어(이미지 뷰어, PDF iframe, HTML5 오디오/비디오 플레이어)를 로드합니다.
* **이진 파일 덮어쓰기 방지 잠금**: 미디어 파일이 활성화된 상태에서 저장 단축키(`Ctrl+S`)나 저장 단추 클릭 시, 에디터 텍스트로 이진 파일을 덮어써 파일이 파괴되는 일을 완벽히 방지하는 저장 잠금 및 경고 토스트를 노출합니다.
...
## ⌨️ Keyboard Shortcuts (단축키 안내)

Joy Markdown Studio는 빠르고 효율적인 문서 작업 및 캔버스 조작을 위해 다양한 단축키를 지원합니다.

### 📝 에디터 및 일반 단축키
* `Ctrl + N`: 서재에 새 마크다운 문서 추가
* `Ctrl + S`: 현재 작업 중인 문서 또는 캔버스 저장
* `Ctrl + Z`: 작업 되돌리기 (Undo)
* `Ctrl + Y` / `Ctrl + Shift + Z`: 작업 다시실행 (Redo)
* `Ctrl + E`: 현재 문서를 단일 HTML 파일로 내보내기
* `Ctrl + P`: 현재 문서를 PDF 포맷으로 인쇄 (미리보기 화면만 깔끔하게 출력)
* `F11`: 문서 전체화면 토글

### 🎨 캔버스 (Canvas) 단축키
* **선택 및 조작**
  * `Ctrl + A`: 캔버스 내의 모든 요소 전체 선택
  * `Shift + 드래그`: 캔버스 영역 드래그로 다중 객체 박스 선택(Marquee)
  * `Shift + 클릭`: 기존 선택을 유지하며 객체 추가/해제 (다중 선택)
  * `Esc`: 모든 선택 해제 (편집 중일 경우 편집 모드 종료)
  * `방향키 (상하좌우)`: 선택된 요소 1px 미세 이동 (Nudge)
  * `Shift + 방향키`: 선택된 요소 10px 이동
  * `Delete` 또는 `Backspace`: 선택된 노드 및 연결선 삭제
* **복제 및 그룹화**
  * `Ctrl + D`: 현재 선택된 요소 20px 간격으로 바로 복제 (Duplicate)
  * `Alt + 드래그`: 드래그 시 해당 객체의 복제본 생성 및 이동 (Clone on drag)
  * `Ctrl + G`: 선택된 요소들을 시각적인 그룹으로 묶기 (Group)
  * `Ctrl + Shift + G`: 선택된 그룹 해제 (Ungroup)
* **화면 제어**
  * `Ctrl + 마우스 휠`: 캔버스 화면 줌 인 / 줌 아웃
  * `Ctrl + 0` 또는 `Shift + 1` / `Shift + !`: 캔버스 내 모든 요소가 화면에 들어오도록 줌 및 위치 맞춤 (Fit to View)
  * `Space + 드래그`: 캔버스 자유 패닝 (이동)
* **텍스트 카드 (마크다운) 편집**
  * `Enter` 또는 `F2`: 선택된 텍스트 카드 즉시 편집 모드 진입
  * `Ctrl + Enter` 또는 `Esc`: 마크다운 편집 완료 및 빠져나오기 (저장)
  * `Ctrl + B`: **볼드(굵게)** 처리 (마크다운 편집 중)
  * `Ctrl + I`: *이탤릭(기울임)* 처리 (마크다운 편집 중)
  * `Ctrl + K`: `[링크텍스트](URL)` 서식 삽입 (마크다운 편집 중)
  * `우클릭 (카드 위)`: 해당 카드의 고유 설정창 및 편집 메뉴 팝업
