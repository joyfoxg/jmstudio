...
### 8. 🖨️ 프리미엄 무설치 PDF 인쇄 및 정적 페이지 엔진 탑재 (v3.9.22 Hotfix)
...
* **자바스크립트 기반 동적 정적 페이지 분할 엔진(Static Pagination Engine) 탑재**: Chromium WebView2의 `@media print` 상에서 `counter(page)` 동기화 누수로 인해 페이지 번호가 `0 / n` 또는 `0 / 0`으로만 표출되는 심각한 웹뷰 렌더러 카운터 오작동을 완벽 우회했습니다. 실제 A4 규격 픽셀 높이를 기준으로 본문 콘텐츠 노드를 동적으로 정확히 계측·분리하여 정적 페이지 구조(`.print-page-wrapper`)로 분할함으로써, **`1 / 2`, `2 / 2` 형태의 완벽한 정적 페이지 번호를 직접 주입**합니다.
* **머리글/바닥글 겹침 원천 차단 및 기하학적 패딩 격리**: 2페이지 이후부터 음수 마진이나 깨진 브라우저 인쇄 영역으로 인해 머리말/꼬리말이 본문 텍스트와 겹쳐서 출력되던 문제를 전면 해결했습니다. 음수 마진 설정을 완전히 폐기하고 머리글 `top: 0`, 바닥글 `bottom: 0`을 고정한 상태에서 여백 강도(넓게/보통/좁게)에 따라 본문 컨테이너에 동적으로 안전 마진(`bodyPaddingTopBottom`)을 정밀 계산 주입하여 완벽한 시각적 격리를 완성했습니다.
...
### 20. 🎨 하이브리드 WYSIWYG 실시간 고급 렌더링 완성 및 안정화 핫픽스 탑재 (v3.9.21 New)
* **모던 다이내믹 요소 실시간 렌더링**: 에디터에 타이핑하여 입력된 Mermaid 다이어그램, 통계 차트(Chart.js), 고급 화학식(SMILES), 9대 서재 템플릿 및 GFM 표(Table) 요소들이 WYSIWYG 모드 상에서 우측 미리보기와 100% 동일하게 완벽 치환 렌더링됩니다.
* **수식 엔터 크래시(JS Error) 소멸**: CodeMirror 6 `RangeSetBuilder`의 엄격한 데코레이션 정렬 규칙을 우회하기 위해 단일 데코레이션 풀링, 오름차순 미세 정렬 및 **사전 겹침 필터링(Overlapping Filtration)** 단계를 새로 도입했습니다. 이로써 수식 입력 후 엔터를 쳐도 에디터 크래시가 절대 일어나지 않는 극상의 연속 신뢰성을 확보했습니다.
* **전체화면 팝업 캡처 및 전역 바인딩 오류 해결**: 줌/전체화면 관련 함수들을 `window` 전역 스코프에 성공적으로 노출하여 사용자가 수동 작성한 HTML 마인드맵의 `onclick` 조건문이 유기적으로 참이 되게 했고, 버튼 내부의 13x13px 크기 부속 아이콘 svg를 낚아채서 팝업창을 가득 메우던 캡처 버그를 `s.closest('button')` 필터링 기법으로 완전히 해결하여 진짜 고화질 마인드맵 본체 SVG만 아름답게 전체화면 팝업됩니다.
